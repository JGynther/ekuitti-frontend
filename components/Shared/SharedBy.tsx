...
const SharedBy: React.FC<any> = ({ eaddressid }) => {
  const { data, isError } = useTokenRequest<Receipts>(`/receipts/shared/${eaddressid}`);
  if (isError) {
  ...
  return (
    <div>
      <div className="px-8 pt-6 pb-4 text-subtitle font-bold">Osoitteesta {eaddressid} jaetut kuitit</div>
